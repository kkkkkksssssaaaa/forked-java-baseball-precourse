...
import baseball.model.PlayerTemplate;
import baseball.model.Player;
import nextstep.utils.Console;

public class Application {
    public static void main(String[] args) {
        PlayerTemplate computer = new ComputerPlayer();

        System.out.println("[TEST] 1부터 9까지의 중복되지 않은 숫자 세 개를 연속하여 입력해주세요.");

        String consoleInput = Console.readLine();
        PlayerTemplate player = new Player(consoleInput);

        while(player.getNumbers().size() == 0) {
